...
vq_parser.add_argument(
    ...
    type=str,
    help="Cut method",
    choices=["original", "updated", "updatedpooling", "latest"],
    default="latest",
    dest="cut_method",
)
vq_parser.add_argument(
...
vq_parser.add_argument(
    "-o", "--output", type=str, help="Output file", default="output.png", dest="output"
)
vq_parser.add_argument(
    "-vid",
    "--video",
...
vq_parser.add_argument(
    ...
    "--rotate_degrees",
    type=float,
    help="Degrees to rotate image, CW: negative CCW: positive",
    default=0,
    dest="rotate_degrees",
)
vq_parser.add_argument(
    ...
    "--pan_horizontal",
    type=int,
    help="Number of pixels to pan left/right",
    default=0,
    dest="pan_horizontal",
)
vq_parser.add_argument(
    ...
    "--pan_vertical",
    type=int,
    help="Number of pixels to pan up/down",
    default=0,
    dest="pan_vertical",
)
...
results_path = f"./results/{args.max_iterations}i_{args.zoom_scale}zsc_{args.rotate_degrees}rd_{prompts_str}"
output_folder = args.output_folder if args.output_folder else results_path
...
def ascend_txt():
    ...
    if args.make_video:
        ...
        filename = os.path.join(output_folder, str(i).zfill(4) + ".png")
        ...
        while True:
            ...
            if args.make_zoom_video:
                if i % args.zoom_frequency == 0:
                    ...
                    img = np.transpose(img, (1, 2, 0))
                    filename = os.path.join(output_folder, str(j).zfill(4) + ".png")
                    imageio.imwrite(filename, np.array(img))
                    ...
                    if args.zoom_start <= i:
                        ...
                        # # Pan, rotate, then zoom
                        # pil_image = pil_image.transform(
                        #     pil_image.size,
                        #     Image.AFFINE,
                        #     (0, 0, args.pan_horizontal, 0, 0, args.pan_vertical),
                        # )
                        pil_image = ImageChops.offset(
                            pil_image, args.pan_horizontal, args.pan_vertical
                        )
                        pil_image = pil_image.rotate(args.rotate_degrees)
