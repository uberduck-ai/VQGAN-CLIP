...
vq_parser.add_argument("-rd",   "--rotate_degrees", type=float, help="Degrees to rotate image, CW: negative CCW: positive", default=0, dest='rotate_degrees')
        ...
        while True:            
            ...
            if args.make_zoom_video:
                if i % args.zoom_frequency == 0:
                    ...
                    img = np.array(out.mul(255).clamp(0, 255)[0].cpu().detach().numpy().astype(np.uint8))[:,:,:]
                    img = np.transpose(img, (1, 2, 0))
                    filename = os.path.join(results_path , str(j).zfill(4) + '.png')
                    imageio.imwrite(filename, np.array(img))
                    ...
                    if args.zoom_start <= i:
                        ...
                        # Rotate and zoom
                        pil_image_rotate = pil_image.rotate(args.rotate_degrees)
                        pil_image_zoom = zoom_at(pil_image_rotate, args.size[0]/2, args.size[1]/2, args.zoom_scale)
